...
def test_customer_phone_index_present_in_db(client):
    ...
    with db_session():
        ...
        present = {ix["name"] for ix in insp.get_indexes("msb_customer")}
    assert "ix_msb_customer_phone" in present, (
        f"ix_msb_customer_phone should exist on the customer table after "
...
def test_unique_constraint_still_present(client):
    ...
    with db_session():
        insp = inspect(db.engine)
        uniques = insp.get_unique_constraints("msb_customer")
...
def test_added_indexes_registry_lists_customer_phone(client):
    ...
    customer_entries = [
        name for name, table, _ in _ADDED_INDEXES if table == "msb_customer"
    ]
