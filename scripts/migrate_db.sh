...
#
# This is the one-shot cutover for the beta customer on dineroexpress1604@
# gmail.com. It's intentionally conservative:
...
psql "$DINERO_URL" < "$BACKUP" >/dev/null
...
echo "   1. Render dashboard → dinerobook service → Manual Deploy → Clear build cache & deploy"
echo "      (so _ensure_added_columns() adds any new columns to the migrated tables)."
echo "   2. Log into https://dinerobook.onrender.com as your test admin to verify."
