...
def _to_login_response(
    ...
    if response is not None and db is not None:
        ...
        issuer = JWTIssuer(
            ...
            permissions=list(result.permissions or permissions_for(
                result.role, db,
                store_id=result.store_id, user_id=result.user_id,
            )),
...
def update_notifications_route(
    ...
    update_notifications(
        ...
        notify_trial_reminders_push=body.notify_trial_reminders_push,
...
def signup_route(
    ...
    perms = permissions_for(
        result.owner.role, db,
        store_id=result.store.id, user_id=result.owner.id,
    )
    from api.Modules.Auth.Services.refresh import (
...
def signup_owner_route(
    ...
    perms = permissions_for(
        result.owner.role, db,
        store_id=result.owner.store_id, user_id=result.owner.id,
    )
    from api.Modules.Auth.Services.refresh import (
        DEFAULT_REFRESH_TOKEN_TTL_SECONDS, issue as _issue_refresh,
    )
